    ...
    , vaccinManager(new Vaccin(this))
{
    ...
    // Initialize the save button for edited data
    ffff=ui->ffff;
    ...
    // In your MainWindow constructor or setup function
    connect(ui->supprimerevent_2, &QPushButton::clicked, this, &MainWindow::on_supprime_eq_clicked);
...
void MainWindow::on_edit_equi_clicked() {
    ...
    if (!selectedItems.isEmpty()) {
        ...
        ui->lineEdit_111->setText(reference);
        ui->lineEdit_114->setText(nom);
        ui->lineEdit_112->setText(type);
        ui->dateEdit_113->setDate(QDate::fromString(dateLimite, "yyyy-MM-dd"));
        ui->lineEdit_115->setText(fournisseur);
        ui->lineEdit_117->setText(quantite);
        ui->lineEdit_116->setText(prix);
...
void MainWindow::on_modif_save_clicked() {
    QString reference = ui->lineEdit_111->text();
    QString nom = ui->lineEdit_114->text();
    QString type = ui->lineEdit_112->text();
    QString dateLimite = ui->dateEdit_113->date().toString("yyyy-MM-dd");
    QString fournisseur = ui->lineEdit_115->text();
    int quantite = ui->lineEdit_117->text().toInt();
    double prix = ui->lineEdit_116->text().toDouble();
...
/*void MainWindow::onTrierParPrixClicked() {
    // Fetch and sort the data by price
    Equipements equipement;
    QSqlQueryModel *model = equipement.afficherEtTrierParPrix();
    ...
    if (model) {
        ...
        QMessageBox::critical(this, "Error", "Failed to retrieve and sort data.");
    }
}
*/
