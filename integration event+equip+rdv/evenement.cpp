...
void Evenement::supprimerEvenement(int id_event) {
    ...
    if (!selectQuery.exec()) {
        QMessageBox::critical(nullptr, "Erreur", "Échec de l'exécution de la requête de sélection : " + selectQuery.lastError().text());
        return;
    }

    if (!selectQuery.next()) {
        QMessageBox::critical(nullptr, "Erreur", "Aucun événement trouvé avec l'ID : " + QString::number(id_event));
        return;
    }
...
void Evenement::sendEmail2(const QString &eventId, const QString &eventName, const QString &eventType,
    ...
    // Générer le corps de l'email en utilisant l'API
    QString prompt = "Rédige un email poli et professionnel pour annoncer l'annulation de l'événement \"" + eventName + "\". "
                                                                                                                        "Commence par 'Cher patient' ou 'Chère patiente'. "
                                                                                                                        "Inclus les détails suivants : "
                                 ...
                                 "Nom de l'événement : " + eventName + ", "
                                   ...
                                   "Date et heure prévues : " + eventDateTime + ", "
                                       ...
                                       "Nombre de participants : " + participantCount + ", "
                                         ...
                                         "À la fin de l’email, ajoute que nous sommes SmartVax.";

    QString body = generateEmailSuppresionBody(prompt);
    qDebug() << "Generated Email Body: " << body;

    // Si le corps de l'email est vide, afficher un message d'erreur
    if (body.isEmpty()) {
        QMessageBox::warning(nullptr, "Erreur", "Le corps de l'email est vide !");
        return;
    }

    // Récupérer les adresses email des patients
    QSqlQuery query("SELECT MAIL FROM PATIENT");
    QStringList receiverEmails;

    while (query.next()) {
        receiverEmails << query.value(0).toString();
    }
    ...
    for (const QString& receiverEmail : receiverEmails) {
...
QString Evenement::generateEmailSuppresionBody(const QString &prompt) {
    QEventLoop loop;
    ...
    request.setRawHeader("x-rapidapi-key", "73c5068138msh191e4a99d121cb3p168b1cjsnb57a7aba7ef6");
