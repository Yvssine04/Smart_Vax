...
void rendez_vous::fetchTunisianHolidays( const QString& year) {
    QString url = QString("https://kayaposoft.com/enrico/json/v2.0/?action=getHolidaysForYear&year=%1&country=tn&holidayType=public_holiday&language=fr")
    .arg(year);
    ...
    connect(manager, &QNetworkAccessManager::finished, this, [=](QNetworkReply* reply) {
        ...
        if (!doc.isObject()) {
            qWarning() << "fomat JSON invalide";
            reply->deleteLater();
            manager->deleteLater();
            return;
        }

        QJsonArray holidays = doc["response"].toObject()["holidays"].toArray();
        ...
        if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            ...
            for (const QJsonValue& val : holidays) {
                ...
                QString date = obj["date"].toObject()["iso"].toString();
                 qDebug() << "Jour:" << name << ":" << date;
                tunisianHolidays.append(qMakePair(name, date));
            }

            file.close();

            qDebug() << "jours fériés enregistrer avec succés:" << file.fileName();
        }
                                 ...
                                 const QString &salle, double facturation, const QString &SelectedDays, const QString &periode)
{
    ...
    if(SelectedDays=="" || periode==""){
        ...
        QSqlQuery qry;
        qry.prepare("SELECT * FROM RENDEZ_VOUS WHERE DATE_RDV = :date AND SALLE_RDV = :salle");
