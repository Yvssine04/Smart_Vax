...
void CustomCalendar::addEvent(QDate date, QString text) {
    if (date.isValid()) {
        // Avoid duplicates by checking if the date already exists
...
void CustomCalendar::paintCell(QPainter *painter, const QRect &rect, const QDate &date) const {
    ...
    for (const SpecialDate &sd : specialDates) {
        if (sd.date == date) {
            ...
            break;  // Stop loop when the date is found
        }
    }
}
...
//void rendez_vous::addNotification() {

 //   notificationWidget->addNotification("Un nouveau patient a été ajaouté n'oublier pas de remplir les informations nécéssaires !");
...
void rendez_vous::infoEdit(int idRDV, QLineEdit *CIN_rdv_2, QDateEdit *daterdv_2, QComboBox *vaccin_3, QLineEdit *adresse_2, QLineEdit *nom_rdv_2, QLineEdit *prenom_rdv_2, QDateTimeEdit *dispo_rdv_2, QComboBox *medecin_att_2, QComboBox *infirmier_att_2, QLineEdit *salle_att_2, QDoubleSpinBox *facturation_2) {
    ...
    if (query.next()) {
        ...
        dispo_rdv_2->setDate(query.value(1).toDate());
