    ...
    , notificationWidget (new NotificationWidget(this))
{
    ...
    QVBoxLayout *layout = qobject_cast<QVBoxLayout *>(calendrier->parentWidget()->layout());

    if (layout) {
        layout->replaceWidget(calendrier, customCalendar);
        delete ui->calendrier_rdv;  // Remove old calendar
    }

    ui->calendrier_rdv = customCalendar;

    // Add event on April 15, 2025
    customCalendar->addEvent(QDate(2025, 4, 15), "Meeting");
