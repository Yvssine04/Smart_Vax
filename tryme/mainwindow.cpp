    ...
    , notificationWidget (new NotificationWidget(this))
{
    ...
    lundiCheckBox = ui->lundi;
    mardiCheckBox = ui->mardi;
    mercrediCheckBox = ui->mercredi;
    jeudiCheckBox = ui->jeudi;
    vendrediCheckBox = ui->vendredi;
    samediCheckBox = ui->samedi;
    ...
    connect(lundiCheckBox, &QCheckBox::stateChanged, this, &MainWindow::onCheckboxChanged);
    connect(mardiCheckBox, &QCheckBox::stateChanged, this, &MainWindow::onCheckboxChanged);
    connect(mercrediCheckBox, &QCheckBox::stateChanged, this, &MainWindow::onCheckboxChanged);
    connect(jeudiCheckBox, &QCheckBox::stateChanged, this, &MainWindow::onCheckboxChanged);
    connect(vendrediCheckBox, &QCheckBox::stateChanged, this, &MainWindow::onCheckboxChanged);
    connect(samediCheckBox, &QCheckBox::stateChanged, this, &MainWindow::onCheckboxChanged);
    ...
    connect(ui->ap_midi, &QCheckBox::stateChanged, this, &MainWindow::onCheckboxChangedP);
    ...
    // 1. Enable drag on source list
    ui->liste_att->setDragEnabled(true);
    ui->liste_att->setSelectionMode(QAbstractItemView::SingleSelection);
    ui->liste_att->setDragDropMode(QAbstractItemView::DragOnly);

    // 2. Enable drop on target list
    ui->liste->setAcceptDrops(true);
    ui->liste->setDragDropMode(QAbstractItemView::DropOnly);
    ui->liste->setDefaultDropAction(Qt::MoveAction);

    // 3. Install event filter to handle the drop
    ui->liste->installEventFilter(this);
...
void MainWindow::handleNotificationClicked() {
...
void MainWindow::onCheckboxChangedP()
{
    ...
    if (ui->lundi->isChecked()) periode= "Lundi";
    if (ui->mardi->isChecked()) periode = "Mardi";
    if (ui->mercredi->isChecked()) periode = "Mercredi";
    if (ui->jeudi->isChecked()) periode = "Jeudi";
    if (ui->vendredi->isChecked()) periode = "Vendredi";
    if (ui->samedi->isChecked()) periode = "Samedi";
